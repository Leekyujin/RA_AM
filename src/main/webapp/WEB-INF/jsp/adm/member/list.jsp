	...
	<div class="container-md main mx-auto px-3">
		<div class="flex">
			...
			<form class="flex">
				...
				<select data-value="${authLevel }" name="authLevel" class="select select-info w-24 max-w-xs">
					<option disabled="disabled">회원 타입</option>
					<option value="3">일반</option>
					<option value="7">관리자</option>
					<option value="0">전체</option>

				</select> <select data-value="${searchKeywordTypeCode }" name="searchKeywordTypeCode" class="select select-info w-24 max-w-xs ml-1">
				...
				<input name="searchKeyword" type="text" class="input input-bordered input-info w-96 max-w-xs ml-1" placeholder="검색어를 입력해주세요"
					maxlength="20" value="${param.searchKeyword }" />
				<button type="submit" class="btn btn-outline btn-success ml-1">검색</button>
			</form>
		</div>
		...
		<div class="float-right mt-1">
			<button class="btn btn-outline btn-error btn-delete-selected-members">선택삭제</button>
		</div>
